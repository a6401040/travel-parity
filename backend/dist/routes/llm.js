...
router.post('/llm/recommendations', async (req, res) => {
    try {
        ...
        const trains = pruneTrains(trainsRaw, { avoidNightArrival: input.constraints?.avoidNightArrival, maxTransfers: input.constraints?.maxTransfers });
        const flights = pruneFlights(flightsRaw, { avoidNightArrival: input.constraints?.avoidNightArrival, minComfort: input.constraints?.minComfort });
        ...
        const payload = {
            ...
            stats: {
                ...
                hotelAvgRating: hotels.length ? Number((hotels.reduce((s, h) => s + (h.rating || 0), 0) / hotels.length).toFixed(2)) : 0,
                poiCount: poi.length
            },
            ...(input.context || {})
        };
...
function cityToPrimaryIata(city) {
    const map = {
        '广州': 'CAN',
        '乌鲁木齐': 'URC',
        '北京': 'PEK',
