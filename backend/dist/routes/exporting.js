...
function buildTOC(data) {
    ...
    toc.push('## 目录');
    toc.push('- [出行方案](#出行方案)');
    if (data?.schemes?.timeFirst)
        toc.push('  - [时间优先](#时间优先)');
    if (data?.schemes?.priceFirst)
        toc.push('  - [价格优先](#价格优先)');
    if (data?.schemes?.balanced)
        toc.push('  - [综合优先](#综合优先)');
    if (data?.hotels)
        toc.push('- [酒店推荐](#酒店推荐)');
    if (data?.routes)
...
function buildScoringAppendix(data) {
    const lines = [];
    lines.push('## 评分明细附录');
    ...
    if (data?.schemes?.timeFirst)
        dump('时间优先', data.schemes.timeFirst);
    if (data?.schemes?.priceFirst)
        dump('价格优先', data.schemes.priceFirst);
    if (data?.schemes?.balanced)
        dump('综合优先', data.schemes.balanced);
    return lines.join('\n');
}
function toMarkdown(data) {
    ...
    const renderSchemes = (title, list) => {
        lines.push(`## ${title}`);
        for (const s of list || []) {
    ...
    if (data?.schemes?.timeFirst)
        renderSchemes('时间优先', data.schemes.timeFirst);
    if (data?.schemes?.priceFirst)
        renderSchemes('价格优先', data.schemes.priceFirst);
    if (data?.schemes?.balanced)
        renderSchemes('综合优先', data.schemes.balanced);
    if (data?.hotels)
        lines.push('## 酒店推荐', ...data.hotels.map((h) => `- ${h.name} | 评分 ${h.rating ?? '-'} | ￥${h.price ?? '-'}`));
    if (data?.routes)
        lines.push('## 热门路线', ...data.routes.map((r) => `- ${r.name} | 天数 ${r.days} | 里程 ${r.totalDistanceKm ?? '-'}km`));
...
router.post('/export/llm/markdown', requireSession, async (req, res) => {
    try {
        const parsed = LLMOutputSchema.parse(req.body);
...
router.post('/export/llm/pdf', requireSession, async (req, res) => {
    try {
        const parsed = LLMOutputSchema.parse(req.body);
        const pref = await getDownloadSettings(Number(req.user.id));
        ...
        if (fontWarn)
            page.drawText(fontWarn, { x: 400, y: 820, size: 8, font });
        // 两列布局
        const colX = [40, 320];
        const colWidth = 260;
        let col = 0;
        ...
        const drawLine = (text) => {
            ...
            if (y < 80) {
                if (col === 0) {
                    ...
                    if (fontWarn)
                        np.drawText(fontWarn, { x: 400, y: 820, size: 8, font });
                    col = 0;
                    y = 800;
                }
            }
