...
router.post('/auth/register', async (req, res) => {
    try {
        ...
        if (msg === 'username_taken')
            return res.status(400).json({ error: 'invalid_params', message: 'username_taken' });
        if (msg === 'email_taken')
            return res.status(400).json({ error: 'invalid_params', message: 'email_taken' });
        if (msg === 'phone_taken')
            return res.status(400).json({ error: 'invalid_params', message: 'phone_taken' });
...
router.post('/auth/login', async (req, res) => {
    try {
        ...
        if (!u)
            return res.status(401).json({ error: 'unauthorized', message: 'invalid_credentials' });
...
router.delete('/auth/account', requireSession, async (req, res) => {
    try {
