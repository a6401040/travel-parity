...
export async function generateRoutes(args) {
    ...
    const enriched = await Promise.all(poi.slice(0, Math.min(100, poi.length)).map(async (p) => {
        let detail = null;
        if (p.poiId) {
            try {
...
function clusterCenters(list, k) {
    const centers = [];
...
function assignToClusters(list, centers) {
...
function nearestCenter(loc, centers) {
    let best = 0;
    let bestd = Infinity;
...
function scheduleByOpenHours(list, startDate) {
    const items = [];
    const morning = [];
    const afternoon = [];
    const evening = [];
...
function pickSlot(open, category, rating) {
    const r = Number(rating || 0);
    if (/\bclosed\b/i.test(open))
        return '下午';
    if (/\b09:|10:/i.test(open))
        return r >= 4.5 ? '上午' : '下午';
    if (/\b20:|21:/i.test(open))
        return '晚上';
    if (String(category || '').includes('博物馆'))
        return '上午';
    if (String(category || '').includes('美食'))
        return '晚上';
    return r >= 4.3 ? '上午' : '下午';
}
function computeClusterScore(buckets) {
    let gaps = 0;
