...
function heuristicMin(kind, distanceKm) {
    if (kind.from === 'airport' && kind.to === 'airport')
        return Math.max(60, distanceKm > 10 ? 120 : 90);
    if (kind.from === 'station' && kind.to === 'station')
        return Math.max(45, distanceKm > 8 ? 75 : 60);
    if ((kind.from === 'airport' && kind.to === 'station') || (kind.from === 'station' && kind.to === 'airport'))
        return Math.max(90, distanceKm > 15 ? 120 : 90);
    return Math.max(30, distanceKm > 5 ? 45 : 30);
}
export async function planTransfer(city, fromName, toName) {
    const from = await geocode(city, fromName);
    const to = await geocode(city, toName);
    if (!from || !to)
        return { minutes: 90, mode: 'unknown' };
    const o = fmt(from);
    const d = fmt(to);
    let bestMin = 1e9;
    ...
    try {
        ...
        const wc = await cacheGet(walkKey);
        ...
        if (!wc)
            await cacheSet(walkKey, walk, Number(process.env.CACHE_TTL_ROUTE || 180));
    }
    catch { }
    try {
        const trKey = `plan:transit:${o}->${d}`;
        const tc = await cacheGet(trKey);
        ...
        if (!tc)
            await cacheSet(trKey, transit, Number(process.env.CACHE_TTL_ROUTE || 180));
    }
    catch { }
    try {
        const drKey = `plan:driving:${o}->${d}`;
        const dc = await cacheGet(drKey);
        ...
        if (!dc)
            await cacheSet(drKey, driving, Number(process.env.CACHE_TTL_ROUTE || 180));
    }
    catch { }
    if (!isFinite(bestMin) || bestMin === 1e9)
        return { minutes: 90, mode: 'unknown' };
    return { minutes: Math.max(30, bestMin), mode: bestMode };
}
export async function planTransferDetailed(city, fromName, toName) {
    const from = await geocode(city, fromName);
    const to = await geocode(city, toName);
    if (!from || !to)
        return { minutes: 90, mode: 'unknown', distanceKm: 0, kind: 'unknown' };
