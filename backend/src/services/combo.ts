...
function minutesBetween(a: string, b: string) {
  ...
  if (isNaN(ta) || isNaN(tb)) return 1e9
  return Math.max(0, Math.round((tb - ta) / 60000))
}

function priceTrain(t: any): number {
  const seats = Array.isArray(t.seatTypes) ? t.seatTypes : []
  const prices = seats.map((s: any) => Number(s.price || 1e9))
  const p = Math.min(...(prices.length ? prices : [1e9]))
  return p === 1e9 ? 0 : p
}

function priceFlight(f: any): number {
  const cabins = Array.isArray(f.cabin) ? f.cabin : []
  const prices = cabins.map((c: any) => Number(c.price || 1e9))
  const p = Math.min(...(prices.length ? prices : [1e9]))
  return p === 1e9 ? 0 : p
}

function comfortTrain(t: any): number {
  const seats = Array.isArray(t.seatTypes) ? t.seatTypes : []
  if (!seats.length) return 65
  const map = (type: string) => (type.includes('商务') ? 85 : type.includes('一等') ? 75 : type.includes('二等') ? 65 : 70)
  const vals = seats.map((s: any) => map(String(s.type || '')))
  return Math.round(vals.reduce((a: number, b: number) => a + b, 0) / vals.length)
}

function comfortFlight(f: any): number {
  const cabins = Array.isArray(f.cabin) ? f.cabin : []
  if (!cabins.length) return 65
  const map = (code: string) => (code.toUpperCase().startsWith('F') ? 90 : code.toUpperCase().startsWith('C') ? 85 : 60)
  const vals = cabins.map((c: any) => map(String(c.code || '')))
  return Math.round(vals.reduce((a: number, b: number) => a + b, 0) / vals.length)
}

function feasibleConnect(arrive: string, depart: string, minMinutes: number) {
...
export async function buildCombos(input: {
  ...
  destination: string
  date: string
  constraints?: { maxTransfers?: number; minComfort?: number; avoidNightArrival?: boolean }
}) {
  ...
  const trains = pruneTrains(trainsRaw as any, { avoidNightArrival, maxTransfers })
  const flights = pruneFlights(flightsRaw as any, { avoidNightArrival, minComfort })
  ...
  try {
    ...
    if (/暴雨|heavy rain|storm/.test(text)) weatherRisk = 6
    else if (/大雪|heavy snow/.test(text)) weatherRisk = 5
    else if (/雨|snow/.test(text)) weatherRisk = 3
    else if (/大风|wind|fog|雾/.test(text)) weatherRisk = 2
  } catch {}

  const schemes: Scheme[] = []
  // 纯火车
  for (const t of trains.slice(0, 30)) {
    const seg: Segment = { mode: 'train', optionId: t.trainNo, departTime: t.departTime, arriveTime: t.arriveTime, price: priceTrain(t) }
  ...
  for (const f of flights.slice(0, 30)) {
    const seg: Segment = { mode: 'flight', optionId: f.flightNo, departTime: f.departTime, arriveTime: f.arriveTime, price: priceFlight(f) }
    const dur = minutesBetween(seg.departTime, seg.arriveTime)
  ...
  for (const f of flights.slice(0, 10)) {
    for (const t of trains.slice(0, 10)) {
      ...
      if (!feasibleConnect(f.arriveTime, t.departTime, Math.max(MIN_CONNECT_SAME_CITY, transfer.minutes))) continue
      const seg1: Segment = { mode: 'flight', optionId: f.flightNo, departTime: f.departTime, arriveTime: f.arriveTime, price: priceFlight(f) }
      const seg2: Segment = { mode: 'train', optionId: t.trainNo, departTime: t.departTime, arriveTime: t.arriveTime, price: priceTrain(t) }
  ...
  for (const s of schemes) {
    ...
    ;(s as any).scoring = { timeScore, priceScore, comfortScore, transferPenalty, riskPenalty, weights: { wTime, wPrice, wComfort, wTransfer } }
  }

  const sortBy = (key: 'time' | 'price' | 'balanced') => {
    const cpy = [...schemes]
    if (key === 'time') return cpy.sort((a, b) => a.totalTimeMinutes - b.totalTimeMinutes).slice(0, 3)
    if (key === 'price') return cpy.sort((a, b) => a.totalPrice - b.totalPrice).slice(0, 3)
