...
router.get('/compiled/recommendations', async (req, res) => {
  try {
    ...
    const center = await (async ()=>{
      try { const g = await geocode({ address: cityName }); const loc = g?.location || g?.data?.location; if (loc){ const [lng,lat] = String(loc).split(',').map(Number); return { lat, lng } } } catch {}
    ...
    let hotelsRaw = await (async ()=>{ try { return await searchHotels({ city: q.city, checkin, checkout: nextDay, budgetMin: undefined, budgetMax: q.budgetMax, ratingMin: q.ratingMin, nearPoi: undefined, limit: q.limit || 50 }) } catch { return [] } })()
    ...
    const MAX_KM = 30
    let hotels = hotelsRaw.map((h:any)=>{
    ...
    hotels = hotels.filter((h:any)=> (h.distanceKm == null || h.distanceKm <= MAX_KM) && (!h.adname || String(h.adname).includes(cityName)))
    let poiRaw = await (async ()=>{ try { return await searchPoi({ city: q.city, interests: q.interests as any, ratingMin: q.ratingMin, limit: q.limit || 60 }) } catch { return [] } })()
    if ((!poiRaw || poiRaw.length===0) && center){
      try {
        const kw = (q.interests && (q.interests as any[]).length) ? (q.interests as any[])[0] : '景点'
    ...
    poi = poi.filter((p:any)=> (p.distanceKm == null || p.distanceKm <= MAX_KM) && (!p.adname || String(p.adname).includes(cityName)))
    const routes = await (async ()=>{ try { return await generateRoutes({ city: q.city, days: 3, interests: q.interests, ratingMin: q.ratingMin, limit: q.limit || 60, startDate: q.date }) } catch { return [] } })()
    const w = await (async ()=>{ try { return await weather({ city: q.city }) } catch { return null } })()
    ...
    let trains:any[] = [], flights:any[] = []
    if (q.origin && q.destination && q.date){
      try { trains = await fetchTrainOptions({ origin: q.origin, destination: q.destination, date: q.date }) } catch {}
      try { flights = await fetchFlightsByCityDate({ date: q.date, fromCity: q.origin, toCity: q.destination, limit: 100 }) } catch {}
    }
    ...
    const budgeted = budgetCap ? hotels.filter((h:any)=> typeof h.price === 'number' ? (h.price <= budgetCap) : true) : hotels
    const recommendedHotels = [...budgeted].sort((a:any,b:any)=>{
