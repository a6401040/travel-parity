...
router.get('/dest/hotels', async (req, res) => {
  try {
    ...
    let list = all.map((h: any) => ({ ...h, distanceKm: dist(h) }))
    if (typeof q.radius === 'number' && loc) list = list.filter((h: any) => (h.distanceKm ?? Infinity) <= (q.radius as number) / 1000)
    if (typeof q.district === 'string') list = list.filter((h: any) => String(h.address || '').includes(String(q.district)))
    if (q.tags && q.tags.length) list = list.filter((h: any) => q.tags!.some((t) => String(h?.tags || h?.name || '').includes(t)))
    if (q.sort === 'distance') list = list.sort((a: any, b: any) => (a.distanceKm ?? Infinity) - (b.distanceKm ?? Infinity))
    if (q.sort === 'rating') list = list.sort((a: any, b: any) => Number(b.rating || 0) - Number(a.rating || 0))
    if (q.sort === 'price') list = list.sort((a: any, b: any) => Number(a.price || 1e9) - Number(b.price || 1e9))
...
router.get('/dest/poi', async (req, res) => {
  try {
    ...
    let list = all.map((p: any) => ({ ...p, distanceKm: dist(p) }))
    if (typeof q.radius === 'number' && loc) list = list.filter((p: any) => (p.distanceKm ?? Infinity) <= (q.radius as number) / 1000)
    if (typeof q.district === 'string') list = list.filter((p: any) => String(p.address || '').includes(String(q.district)))
    if (q.tags && q.tags.length) list = list.filter((p: any) => q.tags!.some((t) => String(p?.category || p?.name || '').includes(t)))
    if (q.sort === 'distance') list = list.sort((a: any, b: any) => (a.distanceKm ?? Infinity) - (b.distanceKm ?? Infinity))
    if (q.sort === 'rating') list = list.sort((a: any, b: any) => Number(b.rating || 0) - Number(a.rating || 0))
    const total = list.length
    const page = q.page || 1
