...
export async function ensureSchema() {
  ...
  await mysql.query(
    `CREATE TABLE IF NOT EXISTS c_trip_data_new (
      id BIGINT NOT NULL AUTO_INCREMENT,
...
export async function ensureSeedUsers() {
  ...
  if (cnt > 0) return
  await mysql.query(
    `INSERT INTO users (username, password_hash, email, phone, status, role, subscription_plan, subscription_expire_at, created_at, updated_at)
     VALUES 
