...
export async function getDownloadSettings(userId: number) {
  ...
  if (!row) return { default_export_format: 'markdown', filename_rule: '{origin}-{destination}-{date}-{ts}', include_segments_detail: 0, download_path: 'Desktop' }
...
export async function upsertDownloadSettings(userId: number, s: { default_export_format: 'json' | 'markdown' | 'pdf'; filename_rule: string; include_segments_detail: boolean; download_path?: string }) {
