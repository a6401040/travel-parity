...
export async function requireSession(req: any, res: any, next: any) {
  try {
    ...
    if (!sid) return res.status(401).json({ error: 'unauthorized', message: '未登录或会话已过期' })
    const key = `session:${sid}`
    const sess = await cacheGet<any>(key)
    if (!sess || !sess.uid) return res.status(401).json({ error: 'unauthorized', message: '未登录或会话已过期' })
...
export function extractSID(req: any): string | null {
...
export async function createSession(uid: number, username: string, role: string): Promise<{ sid: string; ttlSeconds: number }> {
  const sid = randomSID()
  const key = `session:${sid}`
